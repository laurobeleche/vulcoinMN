...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"time\" : n,             (numeric) local time transaction entered pool in vlconds since 1 Jan 1970 GMT\n"
    ...
    if (fVerbose) {
        ...
        BOOST_FOREACH (const PAIRTYPE(uint256, CTxMemPoolEntry) & entry, mempool.mapTx) {
            ...
            const CTxMemPoolEntry& e = entry.vlcond;
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "     ,...\n"
            "  ],\n"
            "  \"time\" : ttt,          (numeric) The block time in vlconds since epoch (Jan 1 1970 GMT)\n"
...
UniValue getchaintips(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex)
        setTips.insert(item.vlcond);
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex) {
        const CBlockIndex* pprev = item.vlcond->pprev;
        if (pprev)
            setTips.erase(pprev);
    }
